...
(function () {
  ...
  function makeArchive() {
    ...
    return {
      ...
      pause: function () {},                // the embed exposes no transport
  ...
  backend.on(function (t) {
    // in verify mode, stop at the end of the line under review so the
    // verifier hears exactly the span they are judging
    if (verifying && vStopAt !== null && t >= vStopAt - 0.05) {
      if (backend.pause) backend.pause();
      vStopAt = null;
    }
  });
  backend.on(tick);
  ...
  /* ---------------------------------------------------------------- verify
   * VERIFICATION MODE -- positive confirmation, not just corrections.
   *
   * The correction form only ever collected NEGATIVE signal, so the archive
   * could never say a line was verified, only that nobody had complained.
   * A benchmark needs the opposite, and 5 fully checked meetings is worth
   * more than scattered confirmations across 2,276 (plan 11.5).
   *
   * Line-at-a-time: play this line's audio, stop at its end, answer. Keyboard
   * driven because the whole value is throughput -- at ~55 seconds of speech
   * per line, the interface must never be the slow part.
   *
   *   y / Enter  correct as written
   *   n          wrong -> opens the existing correction form
   *   u          unsure (skip, counted separately, never as verified)
   *   Esc        leave verify mode
   *
   * Progress lives in localStorage per video so a session survives a reload,
   * and Export writes the JSON the ledger ingests. Nothing is submitted
   * silently: this is a local record until the verifier chooses to hand it
   * over.
   */
  var VERIFY_KEY = "mt-verify-" + (mount.getAttribute("data-video-id") || "x");
  var verifying = false, vIndex = -1, vStopAt = null, vState = {};

  try { vState = JSON.parse(localStorage.getItem(VERIFY_KEY) || "{}"); } catch (e) { vState = {}; }
  ...
  function vGoto(i) {
    if (i < 0 || i >= lines.length) { vFinish(); return; }
    vIndex = i;
    // stop where the NEXT line starts; the last line runs to the end
    vStopAt = (i + 1 < starts.length) ? starts[i + 1] : null;
    scrollLineIntoView(lines[i]);
    vRender();
    backend.seek(starts[i]);
  }
  ...
  function vFinish() {
    ...
    vIndex = -1;
    vStopAt = null;
    if (vToggle) vToggle.textContent = "Verify…";
    vRender();
  }
  ...
  vToggle.addEventListener("click", function () {
    verifying = !verifying;
    vToggle.textContent = verifying ? "Stop verifying" : "Verify…";
    if (verifying) {
      vBar.classList.add("mt-verify-on");
      var first = 0;
      while (first < lines.length && vState[first] && vState[first].v) first++;
      vGoto(first);
    } else {
      vBar.classList.remove("mt-verify-on");
      vFinish();
    }
  });
